...
while True:
    ...
    imgRegion = cv2.bitwise_and(img, imgMask, dst=None, mask=None )
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            if currentClass == 'car' or currentClass == 'bus' \
                    or currentClass == 'truck' or currentClass == 'motorbike' and conf > 0.3:
                cvzone.putTextRect(img, f'{classNames[cls]} {conf}', (max(0, x1), max(35, y1)), scale=0.5, thickness=1,
                                   font=cv2.FONT_HERSHEY_COMPLEX, offset=5, colorB=(0, 255, 0))
                cvzone.cornerRect(img, (x1, y1, w, h), l=8)
